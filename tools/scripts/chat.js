...
// 解析房间号：支持 /<roomId> 与 /room/<roomId>
function getRoomId() {
  const parts = window.location.pathname.split('/').filter(Boolean);
  if (parts[0] === 'room' && parts[1]) return parts[1];
  return parts[0] || '';
}
// 读取分享链接里的 ?password=（存在则直接进入，不必手输）
function getUrlPassword() {
  try {
    const v = new URLSearchParams(window.location.search).get('password');
    return v ? v.trim() : '';
  } catch (e) { return ''; }
}
...
function initPage() {
  ...
  if (roomId) {
    const urlPwd = getUrlPassword();
    if (urlPwd) {                     // 链接自带密码：直接进房间
      roomPassword = MD5(urlPwd);
      connectWebSocket();
      return;
    }
...
function submitRoomPassword() {
  ...
  roomPassword = roomPassword ? MD5(roomPassword) : '';   // 留空 = 无密码房间
...
function redirectToRoom(roomId, roomPassword) {
  // 密码用明文放在链接里（进入后前端会 MD5），方便把整条链接发给对方
  window.location.href = `/tools/chat.html?room=${roomId}&password=${roomPassword}`;
}document.addEventListener('DOMContentLoaded', () => {
